...
export default function AddWalletModal ({isOpen}: ModalProsType) {
  const {getAllWallets, isLoading, errorMessage, createWallet, allWalletData, setShowModal} = useAppContext()
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalWrapper>
        ...
        <ModalContainer>
          {isLoading ? <Wrapper><Loader size={70} width={6} /></Wrapper> : errorMessage ? <Wrapper><ErrorCard onClick={handleTryAgain} loading={isLoading} /></Wrapper> : 
          <ModalBody>
